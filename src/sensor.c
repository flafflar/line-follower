...
void sensor_normalize(uint16_t values[8], uint16_t min, uint16_t max,
                      uint16_t norm[8]) {
...
void sensor_apply_calibration(uint16_t values[8], uint16_t min[8],
                              uint16_t max[8], uint16_t calibrated[8]) {
  for (int s = 0; s < 8; s++) {
    if (values[s] <= min[s]) {
      calibrated[s] = 0;
